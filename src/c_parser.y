...
DECLARATION : VARTYPE_INT DECLARATOR TOK_SEMICOLON { $$ = new DeclareVar("int", $2); }
         ...
         | VARTYPE_INTSTAR DECLARATOR TOK_EQASSIGN ASSIGN_EXPR TOK_SEMICOLON {$$ = new InitMemory("int*", $2, $4);}
...
SINGLE_DECL : VARTYPE_INT DECLARATOR { $$ = new DeclareVar("int", $2); }
            | VARTYPE_INTSTAR DECLARATOR {$$ = new DeclareMemory("int*", $2);}
            ;

VARTYPE_INTSTAR : VARTYPE_INT TOK_MUL;
...
UNARY : FACTOR      { $$ = $1; }
      ...
      | TOK_BIT_AND UNARY    {$$ = new MemoryOperator($2);}
      | TOK_MUL UNARY  {$$ = new DereferenceOperator($2);}
      ;
...
REC_CASE : SWITCH_STATEMENT {$$=$1;}
         | REC_CASE SWITCH_STATEMENT {$$= new RecExpr($1, $2);}
         ;
...
STATEMENT : EXPR_STATEMENT {$$=$1;}
          | ITER_STATEMENT {$$=$1;}
          | JUMP_STATEMENT {$$=new SingleExpr($1);}
          | SELECT_STATEMENT {$$=$1;}
          | FUNC_STATEMENT {$$=$1;}
          //| SWITCH_STATEMENT {$$=$1;}
          //| REC_CASE {$$=$1;}
...
SWITCH_STATEMENT : TOK_CASE COND_EXPR TOK_COLON REC_STATEMENT {$$ = new CaseStatement($2, $4);}
                 | TOK_DEFAULT TOK_COLON REC_STATEMENT {$$ = new DefaultStatement($3);}
                 ;

EXPR_STATEMENT : ASSIGN_EXPR TOK_SEMICOLON {$$= new SingleExpr($1);} //TOK_SEMICOLON 
...
SELECT_STATEMENT : TOK_IF TOK_LBRACKET LOGIC_EXPR TOK_RBRACKET STATEMENT   {$$ = new IfStatement($3, $5);} 
                 ...
                 | TOK_SWITCH TOK_LBRACKET LOGIC_EXPR TOK_RBRACKET TOK_LCBRACKET REC_CASE TOK_RCBRACKET {$$= new SwitchStatement($3, $6);}
                 ;
